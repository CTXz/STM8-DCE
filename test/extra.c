#include <stdint.h>

/**
 * These aren't used! They are here to test that the tool can
 * distinguish between same named local variables in other files.
 */
volatile static const int LOCAL_CONSTANT = 789;
volatile static const int LOCAL_EXCLUDED_CONSTANT = 987;

const uint8_t EXTERNAL_CONST_ARRAY[] = {
    
};

void external_function_sub(void) {
    __asm__("nop");
}

void external_function(void) {
    external_function_sub();
}

static void local_excluded_function(void) {
    __asm__("nop");
}

static void local_function(void) {
    __asm__("nop");
}